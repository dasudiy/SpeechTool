...
namespace SpeechTool.Classes
{
    ...
    {
        public static XElement BuildProsody(int rate, int pitch, params object[] parameters) => new XElement("prosody", new List<object>(parameters) { new XAttribute("rate", $"{rate}%"), new XAttribute("pitch", $"{pitch}%") });
        ...
        public static XElement BuildVoice(string name, string style, string role, params object[] parameters) => new XElement("voice", new List<object>(parameters) { new XAttribute("name", name) });
